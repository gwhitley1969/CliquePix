...
- [ ] **Install-aware QR — iOS, app NOT installed (added 2026-05-13)** — uninstall Clique Pix on an iPhone. Scan invite QR with iOS Camera → mobile Safari opens `clique-pix.com/invite/{code}` → install banner is HIDDEN on iOS (App Store listing not yet live; Smart App Banner activates in Phase C) → only the existing "Sign in to accept" CTA renders → tap → MSAL hosted page → sign in → returns to `/invite/{code}` → web auto-join succeeds. User joined the clique via the web flow. **Acceptance:** no iOS install path advertised until App Store listing is live; iOS users always get the web path. After App Store goes live, this row updates to test the Smart App Banner native flow.
