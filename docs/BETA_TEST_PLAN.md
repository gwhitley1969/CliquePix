...
- [ ] **Apple reviewer account (added 2026-05-06)** — `vwhitley1967@gmail.com` on real iPhone, fresh install → lands on Events with seeded "Apple Review Demo" Clique visible → tap event → sample photos and video render → react ❤️ on a photo → send a DM → sign out and sign back in.
...
- [ ] **Sign-out de-registers FCM token (regression, added 2026-06-04)** — Device A signs out. Then, from another account in a shared clique/event, trigger a push to A (e.g. upload a photo to a shared event A's signed-out user belonged to). Device A's (now signed-out) device receives NO push. Confirm `DELETE /api/push-tokens` fired during sign-out (debug logs) before the JWT was cleared. Same check on Delete Account.
- [ ] **Cross-account data leak regression — sign-out → different sign-up shows EMPTY state (added 2026-05-06)** — sign in as User A on the device. Create a Clique "LeakTest" + Event "ShouldNotLeak" + upload at least one photo. Confirm photo renders. Profile → Sign Out → confirm. Tap Get Started and sign UP as User B (different email — DO NOT sign back in as User A). On the first frame after User B's sign-up: Events tab shows EMPTY state ("No events yet — create your first"); Cliques tab shows EMPTY state. **FAIL** if "ShouldNotLeak" appears anywhere or any User A photo renders. Then sign out as User B and sign back in as User A — User A's "ShouldNotLeak" event + photo MUST still appear (the fix must not break Welcome Back / same-user re-sign-in). Verify in `flutter logs` (debug builds) that `[AUTH] invalidating user-scoped state on identity change` prints exactly once on User A's sign-out and not on User B's sign-in. Verified on iPhone (UDID `00008120-001965E014C3601E`, iOS 26.4.2) 2026-05-06.
...
- [ ] **Stale-token cold-start does NOT leak raw DioException (added 2026-05-03)** — return after 12+ hours OR force-expire the session per the row above. Cold-launch the app. The Events shell may render the friendly error widget ("Couldn't load events. Pull to refresh.") for a frame, but **MUST NOT** show the raw `"DioException [bad response]: This exception was thrown..."` text anywhere. Within ~1-2 frames the WelcomeBackDialog appears (interceptor-triggered fast path). App Insights shows `welcome_back_shown { source: 'interceptor', reason: 'AADSTS700082' }`. **Failure mode to watch for:** any verbatim "DioException" text on screen. If present, regression in `home_screen.dart` (someone reverted to `error.toString()`) — re-apply `friendlyApiErrorMessage` import and call
...
- [ ] **Install-aware QR — Android, app NOT installed (added 2026-05-13)** — uninstall CLIQUE Pix on Device B. Device A generates QR for a Clique. Device B scans QR with system camera → Chrome opens `clique-pix.com/invite/{code}` → install banner renders below "You've been invited" with the "Get on Google Play" badge → tap badge → Play Store opens at the CLIQUE Pix listing → install. On first app launch after install: app routes directly to `JoinCliqueScreen` with the code pre-filled and join completes silently → user lands in the clique detail screen. App Insights: `web_invite_install_banner_shown{platform: 'android'}`, `web_invite_install_badge_clicked{platform: 'android'}`, then on the Flutter side `install_referrer_read{had_invite_code=true}` + `install_referrer_auto_join_attempted`. See `docs/INVITE_INSTALL_REFERRER.md` for the full architecture.
- [ ] **Install-aware QR — iOS, app NOT installed (added 2026-05-13, TestFlight interim)** — uninstall CLIQUE Pix on an iPhone. Scan invite QR with iOS Camera → mobile Safari opens `clique-pix.com/invite/{code}` → install banner renders with the TestFlight badge and the caption "iOS is in public beta on TestFlight. After installing, tap your invite link again to join the Clique." → tap badge → opens `testflight.apple.com/join/hWznNvJ6` → install TestFlight from App Store (if needed) → accept CLIQUE Pix beta → app installs → user opens CLIQUE Pix → signs in → lands on Home (no invite code preserved on iOS — no Apple equivalent of Play Install Referrer). User then retaps the original invite link from Messages → Universal Link routes into the app → JoinCliqueScreen → joins. App Insights: `web_invite_install_banner_shown{platform: 'ios'}`, `web_invite_install_badge_clicked{platform: 'ios'}`. **Future:** when the public App Store listing goes live, swap to AppStoreBadge + activate Smart App Banner meta tag (Phase C-final in `docs/INVITE_INSTALL_REFERRER.md`).
- [ ] **Install-aware QR — desktop browser (added 2026-05-13)** — open `clique-pix.com/invite/{code}` on a desktop browser → install banner renders with the Google Play badge and the benefit bullets → sign-in CTA below still works → join via the web. **Acceptance:** desktop sees the same banner as Android because UA detection falls through to non-iOS.
- [ ] **Install referrer simulated (added 2026-05-13)** — on a connected Android device, run `adb shell am broadcast -a com.android.vending.INSTALL_REFERRER -n com.cliquepix.clique_pix/com.google.android.finsky.installer.BroadcastReceiver --es referrer "invite_code=TESTCODE123"` BEFORE first launch of a fresh install of the AAB. Sign in. App should navigate to `/invite/TESTCODE123`. Note: this synthesizes the Play Install Referrer locally; not needed for actual Play installs.
...
- [ ] **DM mark-read clears unread (regression, added 2026-06-04)** — account B has an unread DM from account A (notification bell / thread shows unread). B opens the thread → the list loads → thread's unread state clears (no 400 in the Network tab on `PATCH /api/dm-threads/{id}/read`; request body carries `last_read_message_id`). Reload the thread list → it stays read. Pre-fix (PR #27) the web client sent no body and the PATCH 400'd, so web mark-read silently failed and the unread state never cleared.
- [ ] **DM own-message alignment (web, regression, added 2026-06-04)** — in a thread on the web, the message you just sent renders on YOUR side (right / 'mine' styling), not as the other participant. Send 2-3 messages and confirm all of your own appear as mine while the other account's appear on the left. Pre-fix (PR #26) the sender's own messages rendered as the other participant because ownership compared the Entra OID to the backend users.id UUID.
...
- [ ] **Web-only member gets in-app notification row (regression, added 2026-06-04)** — account B uses ONLY the web client (never installed the mobile app, so no FCM token). In a shared clique, account A performs an action that notifies members (e.g. join the clique, or upload a photo to a shared event). On B's web Notifications bell, a new in-app row appears (polling + Web PubSub) even though B has no FCM token. Pre-fix (PR #23 NOTIF-1) the notification INSERT lived inside the push-token guard, so web-only members got no row at all.
