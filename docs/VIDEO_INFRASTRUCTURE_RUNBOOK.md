...
- `--replica-retry-limit 1` — retry once within a single execution before that replica fails. **NOTE: there is no automatic poison queue.** The bare `@azure/storage-queue` SDK does NOT auto-dead-letter (that is an Azure Functions trigger-binding feature, not a Storage Queue property), so a persistently-failing or malformed message would otherwise redeliver forever, respawning a 2-vCPU replica every poll cycle. Redelivery is bounded in application code instead: the transcoder runner enforces `MAX_DEQUEUE_COUNT = 5` (`queueService.ts`) — on the 5th delivery it posts a terminal `TRANSCODE_POISON` callback (row → `rejected`, uploader notified, blob eligible for cleanup) and deletes the message. Malformed/undecodable or required-field-missing messages are deleted and dropped on dequeue. See `backend/transcoder/src/queueService.ts` + `runner.ts` (shipped in transcoder image v0.1.8).
...
Failure handling lives in application code, not in Storage Queue / Container Apps config:

- **No auto-dead-letter:** the bare `@azure/storage-queue` SDK does not move failing messages to a poison queue. The runner bounds redelivery itself via `MAX_DEQUEUE_COUNT = 5` (terminal `TRANSCODE_POISON` callback → row `rejected`, then message deleted). Malformed/field-missing messages are dropped on dequeue. (transcoder v0.1.8)
- **Expiry vs in-flight transcode (TQ-2):** the 15-min expiry timer (`cleanupExpired`) will NOT hard-delete an event that still has media in `active`/`pending`/`processing` — deleting mid-transcode would CASCADE-drop the `photos` row and orphan the HLS/fallback/poster blobs the transcoder is about to write (no row ⇒ no cleanup path). 'processing'/'pending' are bounded by the `MAX_DEQUEUE_COUNT` poison guard, so this can't pin an event open forever.
- **Missing-row callback (TQ-2):** if the callback arrives after the row is already gone (event hard-deleted, or uploader/organizer deleted the video), `videoProcessingComplete` returns **200 with `status: 'discarded_no_row'`** and best-effort prefix-deletes whatever blobs the callback reported — it does NOT 404. A 404 would make the runner re-run the whole job and re-upload orphan blobs. Seeing `video_processing_discarded_orphan_row` telemetry is expected, not an incident.
...
| `v0.1.7` | 2026-05-04 | **Source rotation handling — fixes iPhone-portrait videos playing sideways on Android viewers.** New `extractRotation` helper reads `Display Matrix` side data (modern iOS) + legacy `tags.rotate`. `canStreamCopy` now requires `rotation === 0` — rotated sources drop to the slow re-encode path so FFmpeg's default autorotate can bake the correct orientation into the output pixels. Slow path adds `-metadata:s:v:0 rotate=0` to suppress the residual legacy atom on the MP4 branch (prevents player double-rotation; no-op on MPEG-TS HLS segments which have no rotation atom). Slow-path scale filter swapped from height-only `scale=-2:min(ih,1080)` to orientation-agnostic `scale=min(1920,iw):min(1920,ih):force_original_aspect_ratio=decrease` (caps long edge at 1920 — without this, iPhone portrait videos would have been crushed to 608×1080 the moment they hit the slow path). New `source_rotation` field on the success callback drives a `sourceRotation` App Insights telemetry dimension. Verified end-to-end on iPhone-uploader → Samsung-viewer 2026-05-04. See `docs/VIDEO_ARCHITECTURE_DECISIONS.md` Decision 16 + `docs/DEPLOYMENT_STATUS.md` 2026-05-04 entry. Image SHA: `sha256:b4da3290aea83393b7d87488eb18725a9c15adc0961e9cabeeffcec2b0cc57f8`. Rollback: `az containerapp job update -n caj-cliquepix-transcoder -g rg-cliquepix-prod --image cracliquepix.azurecr.io/cliquepix-transcoder:v0.1.6`. |
| `v0.1.8` | 2026-06-04 | **TQ-1 poison-message guard.** The bare `@azure/storage-queue` SDK does NOT auto-dead-letter, so a persistently-failing or malformed transcode message redelivered forever, respawning a 2-vCPU replica each poll. Adds `MAX_DEQUEUE_COUNT = 5` (`queueService.ts`): on the 5th delivery the runner posts a terminal `TRANSCODE_POISON` callback (row → `rejected`) and deletes the message. Malformed/undecodable JSON and required-field-missing messages (`videoId`/`blobPath`/`eventId`/`cliqueId`) are deleted + dropped on dequeue instead of crashing the process. Corrects the prior false 'Storage Queue has built-in DLQ' assumption. Deployed by retagging the CI-built image via `az acr import` (CI build `acf108d…` → `:v0.1.8`), then `az containerapp job update -n caj-cliquepix-transcoder -g rg-cliquepix-prod --image cracliquepix.azurecr.io/cliquepix-transcoder:v0.1.8`. Rollback: `--image …:v0.1.7`. See `docs/DEPLOYMENT_STATUS.md` 2026-06-04 entry. |
