...
| Platform | URI |
|---|---|
| Android (release / Play App Signing — **default**) | `msauth://com.cliquepix.clique_pix/4FsaiJ4wJWgM09R%2FhUh3osYJhgg%3D` |
| Android (debug keystore — local dev) | `msauth://com.cliquepix.clique_pix/W28%2BgAaZ9fNu1yL%2FGMRe94rK0dY%3D` |
...
**Android redirect-URI build-time selection (since 2026-06-04, AUTH-1):** the hash MSAL actually sends is `MsalConstants.androidRedirectUri = String.fromEnvironment('MSAL_ANDROID_REDIRECT_URI', defaultValue: <release hash>)`. Release/CI builds get the Play App Signing hash even if the dart-define is forgotten (fail-safe toward production-correct); local debug builds opt into the debug hash via `flutter run --dart-define-from-file=dart_defines/debug.json`. BOTH hashes are registered as redirect URIs in the Entra app registration AND as `<data>` paths in `AndroidManifest.xml`, so the callback is caught whichever cert signs the running build. These are public cert fingerprints, not secrets. Regenerate the release hash from the Play App Signing cert: `openssl sha1 -binary <cert.der> | openssl base64`. See `docs/SECURITY_AUDIT_2026-06-04.md` (AUTH-1 / H4).
...
ProfileScreen "Sign Out" tap
  ...
  → AuthNotifier.signOut()
     → _deregisterPush() → DELETE /api/push-tokens { token }   (BEFORE JWT clear,
        so this device stops receiving pushes for the signed-out user — H6,
        2026-06-04; best-effort, never blocks sign-out)
  ...
  → ref.listen at app.dart sees previous=userA, next=null → fires _invalidateUserScopedState
     ├─ Invalidates the user-scoped providers (events × 3, cliques × 3, photos × 3,
     │  videos × 4, notifications × 1, DM × 3) PLUS `optimisticEntitlementProvider`
     │  — see app.dart:_invalidateUserScopedState
     └─ PaintingBinding.instance.imageCache.clear() + clearLiveImages()
  → GoRouter redirects to /login
```

`deleteAccount()` runs the same `_deregisterPush()` call before `DELETE /api/users/me`. The callback is injected (not a typed field) to avoid an import cycle with `PushNotificationService`.

**Why the invalidation step matters (since 2026-05-06):** without it, the Riverpod AsyncNotifiers / FutureProviders that hold User A's events, cliques, photos etc. retain their state across sign-out. If a different User B then signs in on the same device, their first frame reads the cached User A state — User B sees User A's content. This was the cross-account data leak fixed 2026-05-06. The listener uses a derived `currentUserIdProvider` (returning `String?`) so it only fires on actual user_id changes, not on AuthState instance churn from background-verify refreshes of the same user. See `DEPLOYMENT_STATUS.md` "iOS cross-account data leak after sign-out → sign-up" for the full root-cause analysis. Since 2026-06-04 (A4) the listener also invalidates `optimisticEntitlementProvider`, so a stale optimistic-entitlement `true` left by a slow RevenueCat webhook at User A's purchase can never grant User B the paid app shell on the same device before the backend reconciles — re-running the provider's create fn rebuilds it with `super(false)`. This fires only on sign-out / account-switch, not on first sign-in.
