...
- **Newly found (web DM mark-read, not yet fixed):** webapp `markThreadRead` PATCHes with no body, but backend `markDmRead` requires `last_read_message_id` and 400s without it → web DM read-receipts silently fail. Surfaced while verifying PR #26.
