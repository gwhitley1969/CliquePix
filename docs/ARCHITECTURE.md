...
The `AppleConfig` in `auth_repository.dart` uses **`Broker.msAuthenticator`** (NOT `safariBrowser`). This routes msal_auth's iOS bridge into its `default:` webviewType branch (`MsalAuthPlugin.swift:235`), which produces an `MSALWebviewType.default` → `ASWebAuthenticationSession` on iOS 13+. The package already sets `prefersEphemeralWebBrowserSession = true` unconditionally at `MsalAuthPlugin.swift:220`, so each interactive sign-in opens a private cookie jar that's destroyed at session end — cookies cannot persist across sign-ins. **`Broker.safariBrowser` was the historical choice but caused an iOS-only account-switching bug**: SFSafariViewController has a per-app PERSISTENT cookie jar that survives `pca.signOut()`, trapping users on a "Continue as <previous user>" prompt at `cliquepix.ciamlogin.com`. The enum name `msAuthenticator` is misleading — for B2C/CIAM tenants, MSAL never brokers via the Microsoft Authenticator app (B2C is unsupported by the broker), so the actual behavior is "use ASWebAuthenticationSession with ephemeral session." `LSApplicationQueriesSchemes` (`msauthv2`, `msauthv3`) in Info.plist is required for the `brokerAvailability = .auto` path that comes with this enum value. No redirect URI is passed — MSAL auto-generates it from the bundle ID.
...
**RC REST client is API v2 ONLY (lockout incident, 2026-06-11):** `backend/src/shared/services/revenuecatRestClient.ts` talks to `https://api.revenuecat.com/v2/projects/{proj}/...`. The Key Vault secret `revenuecat-secret-api-key` is a **v2 project key** — RC's v1 endpoints reject it with error 7723 ("secret API key incompatible with RevenueCat API V1"). The original v1 implementation therefore failed on EVERY production call invisibly (both operations are best-effort and swallow errors): `forceSyncFromRcApi` never actually synced, and the GDPR `deleteSubscriberFromRc` never actually deleted RC customers. The client's `fetchPlusStateFromRc` reads `/customers/{id}/subscriptions`, filters on `gives_access` + entitlement `lookup_key='plus'`, and distills to `{ active, expiresAtMs, productId, store }`; a 404 means "customer unknown to RC" (definitively inactive), distinct from `null` (API error → keep stored DB state).

**RC-API-lag lockout guard (security audit H1, 2026-06-04; extended to lifetime grants 2026-06-04):** `forceSyncFromRcApi` (the `POST /api/users/me/entitlement/refresh` recovery path) must NEVER down-grade an entitled user via a synthetic event. An active `plus` with **no end date** (`expiresAtMs === null` from the v2 client) is a **lifetime/promotional grant** (the mandated App Store reviewer + beta-tester mechanism) and is treated as active-forever; a non-null expiry is active only if still in the future. RC's REST API is eventually-consistent and lags webhook delivery by seconds–minutes — exactly the window when the client's post-purchase auto-recovery calls it. If the DB already shows `active` AND (`entitlement_expires_at IS NULL` for a lifetime/promo grant **OR** a future expiry for a just-paid subscriber), the sync skips deactivation and keeps the webhook-driven state (the 6h reconciliation timer handles genuine subscription expiry). A synthetic EXPIRATION would otherwise win the ordering guard AND make the real RENEWAL webhook get rejected as stale — a sticky lockout (and an App Store reviewer-rejection risk). Synthetic events are RENEWAL-only. See `docs/SECURITY_AUDIT_2026-06-04.md`.

**Webhook user resolution order (lockout incident, 2026-06-11):** `upsertEntitlement` resolves the target user as the **FIRST valid UUID among `app_user_id` → `original_app_user_id` → `aliases[]`**. RC pins `original_app_user_id` to the first id the customer ever had — `$RCAnonymousID:...` **forever** for SDK-created customers, even after `Purchases.logIn` aliases in our `users.id`. The original `original_app_user_id ?? app_user_id` preference therefore dropped EVERY webhook for an anonymous-origin customer as `invalid_user_id` — store purchases self-healed only via the client's 30s REST force-sync (which was itself broken, see above); dashboard promotional grants (the reviewer/beta mechanism) and server-driven renewals/expirations were silently lost. Confirmed live in App Insights with the reviewer's lifetime grant. Do not "simplify" back to a single field.

**Webhook retry-storm + markExpired TOCTOU hardening (PAY, 2026-06-04):** the `POST /api/internal/revenuecat-webhook` handler returns HTTP 200 on every path except an authentication failure (a missing `REVENUECAT_WEBHOOK_SECRET` throws a loud 401) — RC retries on any non-2xx, so an unexpected 500 would mask real subscription events in a retry-storm. An event with NO valid-UUID candidate id (customer never aliased to a `users.id`) is rejected by `upsertEntitlement` as a clean logged no-op (`entitlement_webhook_invalid_user_id`) BEFORE the UUID-typed `WHERE id=$1` UPDATE, rather than raising a Postgres uuid-cast error into the 500 path. Separately, `markExpired` is self-validating to close a TOCTOU: its UPDATE requires `entitlement_active=TRUE AND entitlement_expires_at IS NOT NULL AND entitlement_expires_at < NOW()`, so a RENEWAL webhook landing between the reconciliation-timer snapshot and the per-row UPDATE cannot deactivate a just-renewed payer, and null-expiry lifetime/promotional grants are left untouched. See `docs/SECURITY_AUDIT_2026-06-04.md`.
...
- **User-tagged bootstrap + identity-change provider invalidation (since 2026-05-06):** the bootstrap providers above carry a sibling `bootstrapUserIdProvider` (overridden in `main()` with `bootstrapState.user.id` when authenticated). The events + cliques `AsyncNotifier.build()` methods watch a derived `currentUserIdProvider` (`Provider<String?>` from `authStateProvider`) and gate the bootstrap on `bootstrapUserId == currentUserId` — if a different user signs in mid-session, the bootstrap is rejected and a fresh API fetch is made. A top-level `ref.listen<String?>(currentUserIdProvider, ...)` in `_CliquePixState.build()` invalidates 16 user-scoped providers + `PaintingBinding.instance.imageCache` whenever the authenticated identity changes. Without this, `AsyncNotifier`s and `FutureProvider.family`s holding User A's events / cliques / photos / videos / DMs / notifications retained their state across sign-out — User B signing up on the same device saw User A's content. This was the iOS cross-account data leak fixed 2026-05-06; see `docs/AUTHENTICATION.md` "Sign out" + Migration history, and `docs/DEPLOYMENT_STATUS.md` for the root-cause analysis.
