...
- **Dead-letter handling:** ⚠️ CORRECTED 2026-06-04 — Azure Storage Queue does **NOT** have a built-in dead-letter queue. Automatic poison-queue move after N dequeues is an **Azure Functions Storage-Queue *trigger* binding** feature, not a property of the queue itself, and the transcoder uses the bare `@azure/storage-queue` SDK (no trigger binding). The original claim here was false and produced an infinite-redelivery hole (a persistently-failing message respawned a 2-vCPU replica every poll cycle forever). The runner now bounds redelivery itself: `MAX_DEQUEUE_COUNT = 5` in `queueService.ts` — at the bound the runner posts a terminal failure callback (→ video `rejected`) and deletes the message; malformed/undecodable messages are deleted on dequeue. See the poison guard in `runner.ts` + `queueService.ts`.
...
**Updated 2026-06-04 (PR #23, TQ-2) — event-expiry/transcode race + missing-row callback.** Q5's row-marked-`deleted` mechanics assume the `photos` row still exists. Two gaps were closed when an event could expire mid-transcode:

1. **Expiry timer no longer reaps in-flight media.** `cleanupExpired` marks an event `expired` only when it has no media in `status IN ('active','pending','processing')` (`backend/src/functions/timers.ts:107-113`). Without this guard, hard-deleting an expired event CASCADE-dropped the `photos` row while the transcoder was still writing, orphaning the HLS/fallback/poster blobs forever (no row ⇒ no cleanup path). `pending`/`processing` are bounded — a wedged transcode is forced terminal by the TQ-1 `MAX_DEQUEUE_COUNT` guard (→ `rejected`, reaped by the 1h failed-video orphan sweep) — so this cannot pin an event open indefinitely.

2. **Callback treats a missing row as a terminal DISCARD, not a 404.** If the row is gone entirely (CASCADE-deleted before the guard landed, or a race), `videoProcessingComplete` no longer 404s — a 404 made the runner re-run the whole job and re-upload blobs that have no DB row. Instead it best-effort prefix-deletes whatever the callback reports it wrote (via `deriveVideoDir`, which strips a trailing `hls/` to land on the `{video}/` dir) and returns 200 (`video_processing_discarded_orphan_row` telemetry; `backend/src/functions/videos.ts:950-967`). This is distinct from the row-still-exists `status='deleted'` path above, which uses `cleanupVideoBlobs`.
