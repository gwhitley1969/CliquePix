...
- **Android:** ✅ **PUBLIC on Google Play production — vc9 passed Google review + released at 100% (2026-06-18)**, the first public release and first with working Android billing. Android billing fully unblocked end-to-end (subscriptions active, RTDN wired). Went straight to public on approval, so the License-testing purchase smoke test was **descoped** (billing validated by real public traffic + RC webhooks). Short tail owed: revert the temporary Admin grant on the RC service account to least-privilege, Android tester promos, and **finish ending the open-testing track** (the 7/3 Console "pause" never persisted — the "Join the beta" card was still live + joinable 2026-07-06; the API can only halt the latest release, done — vc8 `halted`, which resurfaced vc7 — full track deactivation is impossible via the API. Gene must Pause track in Play Console and verify via the opt-in URL `play.google.com/apps/testing/com.cliquepix.clique_pix` + the live listing — see `docs/DEPLOYMENT_STATUS.md` "PLAY 'JOIN THE BETA' STILL LIVE (third act)"). See `docs/GENE.md` / `docs/DEPLOYMENT_STATUS.md`.
