...
- **Stale-while-revalidate Home + Cliques.** The Home screen renders the user's last-seen events and cliques from a local cache on the very first frame after cold start. A background refresh fires immediately and updates the list when fresh data arrives, with an inline "Refreshing…" pill. Pull-to-refresh forces it. On a true first install (no cache yet) a 3-card shimmer placeholder shows for the duration of the first network call — never a full-screen blocking spinner.
...
- Vibrant marketing surface with gradient brand accents, phone mockup with live tappable reactions, feature grid, use cases, QR code for laptop-to-phone jump, App Store + Google Play badges (live store URLs since 2026-07-03 — both listings public; the former "Now in beta" chips were removed 2026-07-03 when the app went public on both stores)
