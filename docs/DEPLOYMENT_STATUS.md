...
Last updated: 2026-07-09 (**AZURE CREDIT TICKET FILED — support case `2607090040008626`, $411.25 Container Apps metering-anomaly credit request.** June MCA invoice `G170253352` posted 2026-07-09 ($1,226.67; the full $449.13 Container Apps charge confirmed ON it under the **Compute** service family — "Containers $19.97" is ACR only; taxonomy gotcha) → §1b gate passed, ticket filed same day via the staged ARM PUT, status Open, replies to bluebuildapps@gmail.com. Known quirk: unescaped `$411` in the staged PowerShell title expanded to empty (immutable title reads ".25"); description amounts intact + corrective communication attached. ⏳ Follow to credit or written denial. Support-plan aside: the $100/mo "Microsoft Azure Support" on the invoice is intentional (Gene keeps it for technical support). See top entry. Prior 2026-07-06: **PLAY "JOIN THE BETA" CARD STILL LIVE — the 2026-07-03 "track paused / ✅ RESOLVED" was premature: the pause NEVER PERSISTED** (not queue-stuck — `googleplay02.png` shows managed publishing OFF, Publishing overview empty, last published Jul 5; the API read the beta release as `completed`). API remediation 2026-07-06 hit two hard limits: release removal (`tracks.update releases:[]`) is **silently discarded** (commits succeed, nothing publishes), and halting applies only to the LATEST release — **vc8 is now `halted` (live)** but that resurfaced **vc7 `completed`**, which cannot be halted (`400 no matching in progress release`) → **the API cannot fully deactivate a track.** Interim state harmless (Play serves the highest eligible vc = production vc13). ✅ **Opt-in CONFIRMED CLOSED same day (`google02.png`): the opt-in URL returns "App not available / account isn't eligible" — nobody can join anymore; the beta is functionally dead.** Remaining: card disappears from the live listing (≤24h + Play Store cache; escalate to Play support only if it outlives 48h) + confirm Console Open testing shows paused after a reload. **Same day: Play listing description scrubbed via API** (Gene-approved: durations → 3/7/14 days, "verified at signup" age claim → per-Terms, open-beta paragraph removed, CLIQUE Pix casing) — verified live via fresh-edit read; App Store description scrub still owed (see the 2026-07-05 duration entry). See top entry. Prior 2026-07-05: **FINOPS PASS: APIM CUT OUT OF THE REQUEST PATH + ACR→Basic + budgets right-sized** — June's $718.69 Clique Pix bill was ~62% the already-mitigated Container Apps incident; post-fix run-rate was ~$238/mo. Structural cuts executed live 2026-07-05: Front Door now routes `api.clique-pix.com` **directly to `func-cliquepix-fresh`** (CORS moved to Function App platform config; transcoder callback repointed through the custom domain; APIM traffic confirmed 0 since 19:10Z; **APIM deletion + FD lockdown pending a 48-72h soak** — it idles as instant rollback until ~2026-07-08), **ACR Standard→Basic**, budgets tightened to $120 (sub) / $100 (RG). New run-rate ≈ **$85/month (−88% vs June)**. ~~⏰ File the ~$411.25 Container Apps credit ticket THIS WEEK~~ (✅ filed 2026-07-09, case `2607090040008626` — see top entry). See top entry. Prior 2026-07-03: **PLAY OPEN BETA ENDED** — the store listing was still showing a "Join the beta" card because the **open testing track was never paused** when vc9 went to production 2026-06-18 (API-verified: beta track live with vc8). Fix: pause the open-testing track in Play Console (Gene, manual) + "Now in beta" chips removed from the web landing page. Runbook lesson: **going to production does NOT end open testing — pause the track as part of any go-public checklist.** See top entry. Same day, earlier: 🎉 **iOS APPROVED + PUBLIC ON THE APP STORE** — **1.0 (12)** approved by Apple 2026-07-02 after the promotional-image deletion; listing live + verified via iTunes lookup: `https://apps.apple.com/us/app/clique-pix-group-pic-sharing/id6766294274`. **CLIQUE Pix is now public on BOTH stores** (Android since 2026-06-18). Same-day follow-through shipped: invite-flow badge TestFlight → App Store, **Smart App Banner Phase C-final activated** (`docs/INVITE_INSTALL_REFERRER.md`), landing-page store badges wired live, HANDOFF launch status flipped. See top entry. Prior 2026-07-02: ⚠️ **THIRD APPLE REJECTION** — iOS **1.0 (11)** rejected on **2.3.2** Accurate Metadata: the IAP **promotional images** (the optional, PUBLIC-facing App Store Promotion slot) had been filled with the app icon on both subscriptions — collateral from the 2.1(b) remediation, which only required the separate App Review Screenshot. Both 1.0(10) findings (2.1(b), 2.1(a)) are confirmed cleared. Fix: promotional images **deleted** from both subscriptions 2026-07-02; resubmit **build 11 as-is** (metadata-only — no code, no binary, no Entra work). See top entry. Prior 2026-06-29: **SECOND APPLE REJECTION** — iOS **1.0 (10)** rejected on **2.1(b)** (IAP subscriptions still not submitted for review — the App Review Screenshot was never uploaded, so the IAPs never left "Ready to Submit") + **2.1(a)** (reviewer tapped the Microsoft sign-in page's "…" troubleshooting ellipsis and saw benign silent-SSO error 50058). The four 1.0(9) findings are otherwise cleared. Runbook to clear it: `C:\Users\genew\.claude\plans\how-do-i-get-cozy-hejlsberg.md`. See top entry. Prior 2026-06-23: APPLE REJECTION REMEDIATION — DOB age gate removed; repo code + docs done, manual portal/store work owed; see top entry. Prior — 2026-06-18: ANDROID PUBLIC + iOS IN REVIEW: ✅ **vc9 passed Google review and is fully released to the public on Google Play (production, 100%)** — CLIQUE Pix's first public release, first with working Android billing. The planned License-testing purchase smoke test was **not performed / descoped** — the app went straight to public on approval, so billing is validated by real public traffic + RevenueCat webhooks rather than a controlled test buy. 🍎 **iOS build (`1.0.0 (9)`) is now in active Apple App Store review** — built on the Mac + uploaded via Transporter; carries the new **branded launch screen** (PR #67). Still owed: revert the temporary Admin grant on the RC service account to least-privilege. Prior 2026-06-17: vc9 AAB (clean, upload-key signed, real `goog_` key + paywall never-blank hardening) pushed to the Play **production** track — first build with working Android billing; Google review pending. ANDROID BILLING fully unblocked: **both Google clocks cleared** — subscriptions-API credential VALIDATED (was a missing "Manage orders and subscriptions" perm) **and** Payments-profile org-name verification (BlueBuildApps→Xtend-AI) approved. Play subscriptions are **created + active** (`plus_monthly`/`monthly` $3.99, `plus_annual`/`annual` $39.99 + 7-day free-trial offer) and **RTDN is wired** (Connect to Google needed a temporary Domain Restricted Sharing org-policy override — see top entry). Remaining: Android purchase smoke test on vc9 + revert the temporary Admin grant to least-privilege. Prior 2026-06-16: RevenueCat Play app created + `goog_` key wired into the client (PR #62, merged to `main`, versionCode bumped to 9); RC products/entitlement/offering fully wired for Android. See top entry. Prior — 2026-06-11: COST INCIDENT transcoder Container Apps Job billed ~$447 MTD, root-caused + mitigated. Prior same-day: app-wide lockout — all 14 users' backfilled trials expired 2026-06-09 and the Android paywall rendered a blank screen [placeholder RC key + no PaywallView fallback]; resolved same-day: trials extended +30d via SQL, reviewer lifetime promo grant live end-to-end, TWO production backend bugs fixed + deployed [webhook app_user_id resolution order, RC REST client on v1 API with a v2-only key], paywall never-blank hardening + stable-router fix on `main` pending next mobile build. Prior: Play rejection of versionCode 6 fixed [PR #52], versionCode 7 uploaded 2026-06-10 — in Google review.)

## AZURE CREDIT TICKET FILED: $411.25 Container Apps metering anomaly → support case 2607090040008626 — ⏳ await Microsoft credit-or-denial (2026-07-09)

**Trigger.** The June MCA invoice **`G170253352`** posted 2026-07-09 (period Jun 1–30, total **$1,226.67 USD**, due same day, card auto-charges) — the gate `docs/MICROSOFT_CREDIT.md` §10/§12 had been waiting on since 2026-07-05.

**§1b gate PASSED before filing (line-level, not just the total):** the invoice PDF groups by *service family*, and the taxonomy is a trap — **"Containers $19.97" is Container Registry only; Azure Container Apps bills under "Compute"**. A billing-profile-scope Cost Management query (ServiceFamily+ServiceName) confirmed the full **$449.13** of Container Apps inside the invoice's Compute $602.39 line (449.13 ACA + 148.54 Functions + 4.75 App Service); every family line reconciles to the invoice within pennies and the grand total sums to $1,226.67 exactly. Microsoft did **not** retroactively correct the June metering → the ticket was necessary.

**Filed 2026-07-09 19:52 UTC** via the staged ARM PUT (Billing → "Disagreement with a charge / Issue with Compute charge", free — no support plan needed): **`supportTicketId 2607090040008626`**, status Open, severity minimal, contact bluebuildapps@gmail.com (auto-ack received). Ask: **$411.25** credit (floor $346.84) for the June 3–9 anomaly excess above the $3.74/day baseline.

**⚠️ Staged-command bug, worked around:** the staged title string had an unescaped `$411.25` in PowerShell double quotes — `$411` expanded to empty, so the immutable ticket title reads "credit request .25". The **description carries every amount intact**, and a corrective communication (`title-amount-correction`) restating the $411.25 ask was attached at 19:53 UTC. Lesson recorded in MICROSOFT_CREDIT.md §12: backtick-escape EVERY literal `$`, titles included.

**⏳ Next:** follow case **2607090040008626** (portal Help + support / bluebuildapps@gmail.com) to an issued credit or a written denial; record the outcome in `MICROSOFT_CREDIT.md` §12. **Aside from the invoice:** the **$100/month "Microsoft Azure Support"** plan (term 06/07→07/06) is **intentional — Gene keeps it for technical support** (billing disputes like this one are free regardless, so it wasn't needed for the ticket).

---

## PLAY "JOIN THE BETA" STILL LIVE (third act): 7/3 pause never persisted; API can only HALT the latest release (vc8 halted, vc7 resurfaced) — ⏳ Gene: Pause track in Console + verify via the opt-in URL (2026-07-06)

**Symptom.** `stillbeta01.png` (2026-07-06): the public Play listing STILL renders the **"Join the beta"** opt-in card with a live **Join** button — three days after the 2026-07-03 "Pause track" action that the entry below recorded as ✅ RESOLVED. The listing is otherwise current (What's New carries the July 5 duration copy, so production vc13 is approved + live). **⚠️ CORRECTION to the 2026-07-03 entry:** it was closed on the Console banner ("This track is paused", summary Inactive) — the live store never reflected it.

**Root cause (established from two evidence sources):**
- **`googleplay02.png` (Gene, Publishing overview):** **Managed publishing is OFF**, no changes queued or in review, "Last published on July 5, 2026" — so nothing was ever stuck in a publish queue. Combined with the API reading the vc8 release as `status=completed` (halted releases DO surface in the API — see below), the conclusion is **the 7/3 Console pause simply never persisted** (banner shown, state not saved).
- Live track state pre-remediation (Play Developer API via the play-store-mcp's own SA key `C:\Dev\clique-pix-d7fde-6786aa684d70.json`, 2026-07-06 ~21:42Z): production `completed` vc13 100% · **beta `completed` vc8 100%** · alpha/internal inactive drafts.

**API remediation attempted (2026-07-06 ~21:45–22:05Z) — two hard API limitations discovered:**
1. **Release removal is silently discarded.** `edits.tracks.update {track:'beta', releases:[]}` + commit, run twice (edit ids `08937862728451996995`, `04878437102255623450`): the update applies in-edit (HTTP 204; in-edit read-back `{"track":"beta"}`, no releases) and both commits return success — **but the live state never changes and no publish event registers** (Publishing overview kept "Last published July 5"). Google's server treats track updates as release *upserts*; a completed release cannot be removed from a track via the API. Commit success ≠ change applied.
2. **Halting works — but only on the latest release, and it resurfaces the previous one.** `tracks.update {releases:[{versionCodes:['8'], status:'halted'}]}` (edit `13206095209360685729`) **went live instantly**: vc8 is now `halted`. But the track then exposed the prior release — **vc7 now sits `completed` (live) on the track** — and halting vc7 is rejected: `400 Cannot halt release, no matching in progress release found` (only the newest release can be halted). Net: **the Play Developer API cannot fully deactivate this track.** Current live track = vc8 `halted` + vc7 `completed`.

**Interim state is harmless to users:** Play always serves the highest eligible versionCode, so anyone joining the beta today still installs production vc13 — the active track's only real-world effect is the "Join the beta" card itself.

**Why direct API and not the play-store-mcp:** the MCP server exposes only `get_releases` / `deploy_app` / `promote_release` — no track management at all. One-off scripts in the session scratchpad (`end-play-beta/*.js`, googleapis npm), not committed to the repo.

**⏳ Gene (Play Console, manual — the actual unblock):**
1. **Test and release → Testing → Open testing** → note what the track shows now (with vc8 halted + vc7 completed it may render as paused or as active-on-vc7) → **Pause track** (or re-pause), and confirm the state SURVIVES a full page reload / Console re-login.
2. ✅ **Acceptance check #1 PASSED (2026-07-06, `google02.png`):** `https://play.google.com/apps/testing/com.cliquepix.clique_pix` (signed-in) returns **"App not available — Your account isn't currently eligible for this app's testing program"** — the program is refusing new testers. **The beta is functionally dead**: even while the listing card lingers, tapping Join dead-ends here.
3. **Acceptance check #2 (≤~24h + clear Play Store app cache):** listing from a non-enrolled account shows NO "Join the beta" card. Then delete `stillbeta01.png` + `googleplay02.png` + `google02.png` (untracked working files).
4. Escalation if the opt-in page refuses joins but the card survives 24–48h: Play Console support ("Submission activity" on Publishing overview documents the publish history as evidence).

**Runbook lesson (supersedes the 2026-07-03 lesson):** (1) The Console "track is paused" banner is NOT evidence — the 7/3 pause showed it and never persisted. Acceptance = the public opt-in URL + the live listing. (2) Play Developer API quirks (all proven here): commit success does not mean the change applied; removing a completed release from a track is silently discarded; only the latest release can be halted, and halting it resurfaces the previous completed release; therefore a track cannot be fully deactivated via the API. (3) `play-store-mcp` has no track-management operations.

**Rollback:** Console → Open testing → Resume track (or upload a newer AAB to the beta track) restores serving. Tester list is untouched throughout.

---

## FEATURE: event duration presets 24h/3d/7d → 3d/7d/14d — code complete, deploy in progress (2026-07-05)

**Product change (Gene):** drop the 24-hour event duration, add 14 days. Offered `retention_hours` become **72 / 168 / 336**; default stays **168 (7 days)**.

**Legacy-24 shim (deliberate, don't "fix"):** installed mobile builds ≤1.0.0+12 have `[24, 72, 168]` compiled in and keep sending 24 — the backend validator AND the DB CHECK (migration `016_retention_14d.sql`) still accept 24 so old builds don't 400. The validator's error message names only 72/168/336. Tighten both in v1.5 once old builds age out (dated comments at `validators.ts` + `event.ts` + the migration).

**What changed:** backend `validateRetentionHours` + `RetentionHours` type (+336, keep 24) + migration 016 (name-agnostic CHECK swap) + validator tests; Flutter `app_constants.dart` presets/labels (rides build **1.0.0+13**; UI picker is data-driven, no screen changes) + test fixture; web `CreateEventModal` presets + create-input union (read-model union keeps 24 for old rows) + **landing copy** (`Features.tsx`, `HowItWorks.tsx` "Pick 3 days, a week, or two weeks"); **legal docs** `terms.html` §4 + `privacy.html` §6/§8 durations updated with **Effective Date bumped to July 5, 2026**; PRD/ARCHITECTURE/CLAUDE/BETA_TEST_PLAN.

**Deploy status (2026-07-05):** ✅ backend `func publish` DONE (health 200) · ✅ **migration 016 APPLIED + verified** (CHECK now `retention_hours IN (24,72,168,336)`; sole existing event at 168 untouched). ⚠️ **Dev-box DB access gotcha (dual-WAN, post-Windstream-switch):** the box's HTTPS egress shows `98.22.9.190` (Windstream) but its **Postgres traffic egresses via `38.125.100.58`** (second WAN path) — psql failed with "server closed the connection unexpectedly" until a firewall rule for `.58` was added (`AllowDevMachineAlt`; `AllowDevMachine` = `.190` kept). `curl ifconfig.me` is NOT a reliable guide for the 5432 path on this network — if psql fails with the rule "matching", allow BOTH egress IPs. Merge gate cleared → PR #74 merged; web deployed + **verified end-to-end** (Gene created a live 14-day event: `retention_hours=336`, exact 14-day `expires_at`, "13 days left" card). **Mobile build `1.0.0+13`** (pubspec bumped 2026-07-05, commit `75362a2`): Android AAB built + uploaded to the Play **production** track — in Google review; publishes at 100% on approval (vc9 pattern). Carries the presets + the DOB age-gate client cleanup (PR #70 — Android never shipped it; iOS has had it since build 10). **iOS: ✅ version 1.0.1 (build 13) submitted to Apple review 2026-07-05** — built on the Mac with `flutter build ipa --release --build-name=1.0.1` (marketing version had to exceed the live "1.0"; pubspec stayed `1.0.0+13`, so there is a deliberate version split: Android ships as 1.0.0 (vc13), iOS as 1.0.1 (13) — reconcile the next cycle by starting from `1.0.1+14` in pubspec). Uploaded via Transporter, version 1.0.1 created in ASC, build 13 attached, submitted — NO IAP pages touched (2.3.2 lesson). Old installed builds covered by the legacy-24 shim indefinitely. ✅ **Play listing description scrubbed 2026-07-06 via the Play Developer API** (verified live with a fresh-edit read): "How it works" durations → "3 days, 7 days, or 14 days"; PLUS three more stale items fixed with Gene's approval — **"13+ minimum age, verified at signup" → "(per our Terms of Service)"** (nothing is verified since the 2026-06-23 age-gate removal — was an inaccurate-metadata risk), the closing **"This is the open beta…" paragraph removed** (public since 2026-06-18), and **"Clique Pix" → "CLIQUE Pix"** (×2, 2026-06-09 rename). Store-surface propagation may take a few hours. ⏳ Still owed: the **App Store** description scrub (ASC, manual — check for the same four staleness classes: durations, age-verification claim, beta language, brand casing).

**Gene manual (non-blocking):** ~~check Play listing description~~ (✅ done via API 2026-07-06 — see above); check the **App Store** description in ASC for "24 hours" duration mentions, "verified at signup"-style age claims, beta language, and "Clique Pix" casing (store copy isn't in the repo).

---

## FINOPS PASS: APIM removed from request path, ACR→Basic, budgets right-sized — ⏳ SOAK until ~2026-07-08, then delete APIM + lockdown (2026-07-05)

**Why.** June 2026 Clique Pix subscription bill: **$718.69** ($449.13 Azure Container Apps — of which ~$411 was the June 3–9 metering anomaly + the rest the `minExecutions=1` idle burn, both handled in June). Post-incident July run-rate was ~$238/mo, of which APIM Basic v2 was ~$148 — and live inspection showed APIM's ONLY remaining function was CORS for two origins (rate limiting removed after the six 429 incidents; JWT auth in Functions code; `subscriptionRequired: false`; 1:1 path pass-through `api` → `.../api`; no custom domain on APIM). Gene approved removing APIM (overriding the CLAUDE.md locked gateway rule), keeping Front Door, keeping Web PubSub S1, keeping the account support plan.

**What was executed live (2026-07-05, ~18:48–19:45 UTC, all verified):**
1. **CORS handover:** Function App platform CORS added (`https://clique-pix.com`, `http://localhost:5173`, allow-credentials false) + APIM API-scope policy replaced with a base-only pass-through, back-to-back. ⚠️ Found during cutover: the APIM API had catch-all operations for GET/POST/PUT/PATCH/DELETE but **not OPTIONS** (the old cors policy had been answering preflights itself), so preflights 404'd at APIM for ~12 minutes (18:51–19:03 UTC, ~0–2 concurrent users, Sunday evening) until a `catch-all-options` operation was added as the interim fix.
2. **Transcoder callback repointed:** `FUNCTION_CALLBACK_URL` → `https://api.clique-pix.com/api/internal/video-processing-complete` (was the raw azurewebsites URL — would break under the FD lockdown). Query-string `?code=` function-key auth preserved through FD.
3. **Front Door origin swap:** new origin group `func-origin-group` (probe GET `/api/health`, 100s) + origin `func-cliquepix-fresh.azurewebsites.net` (originHostHeader = same); route `default-route` repointed. **APIM request rate collapsed ~95/5min → 0 at 19:10 UTC** and stayed 0 — cutover complete. Rollback at any time: repoint route at `apim-origin-group`.
4. **Validation sweep (all green):** health 200 · preflight 204 for both allowed origins with correct headers · foreign origin gets NO `Access-Control-Allow-Origin` · `POST /api/auth/verify` 401 canonical · RC webhook 401-on-bad-auth via the domain · internal callback endpoint 401-without-key via the domain.
5. **ACR `cracliquepix` Standard → Basic** (stores 265 MB of what was a 100 GB allowance; ~$14/mo saved). CI's `gh-actions-transcoder` ACR token + scope map verified intact post-downgrade.
6. **Budgets right-sized:** subscription budget `Clique_Pix` $450→$120 (actual>100% + forecast>90%), RG budget `budget-cliquepix-video` $450→$100 (actual>85%) — at an $85 run-rate, $450 thresholds would have let a June-style anomaly burn ~6 days before alerting. The daily cost-anomaly alert (created 2026-06-22 post-incident) verified Enabled, notifying bluebuildapps@gmail.com + gwhitley@xtend-ai.com + genewhitley2017@gmail.com.

**⏳ Pending — post-soak (~2026-07-08, after 48–72h of clean telemetry + Gene's smoke tests):**
1. **Lockdown:** `az functionapp config access-restriction add ... --rule-name AllowFrontDoorOnly --service-tag AzureFrontDoor.Backend --http-headers x-azure-fdid=4e41fded-8d53-4ecd-bc17-af06024ecfad` (SCM site stays unrestricted — deploys). Then re-verify: transcode round-trip, RC test webhook, `func publish`.
2. **Delete:** `apim-429-detected` alert → `apim-cliquepix-003` (soft-deletes; optional purge) → FD `apim-origin-group`.
3. **Gene smoke (owed):** web sign-in + photo upload from a real browser, mobile smoke, **one test video upload** (proves the new callback URL end-to-end — nothing has exercised it yet).

**⏰ Gene manual — file the credit ticket THIS WEEK:** June MCA invoice posts ~July 5–9; `docs/MICROSOFT_CREDIT.md` is the complete evidence package (ask $411.25, floor $346.84, free Billing ticket). Record the ticket number here + in MICROSOFT_CREDIT.md.

**Money:** run-rate ~$238 → **~$85/month** (WPS S1 $48 + FD ~$31 + Postgres ~$16 + ACR $5 + misc; −88% vs June's $718.69, −64% vs the post-incident run-rate) + ~$411 one-time credit if granted. Deferred/declined: WPS S1→Free (−$48, revisit ~August — peak was 2 concurrent connections vs Free's 20 cap), FD removal (−$34, declined), support plan (−$100 account-level, declined), Postgres 1-yr RI (−$4–5, optional). **Next-biggest target (separate effort): My AI Bartender subscription ($408/mo) — its own ~$148 APIM + ~$148 Functions line (likely an idle Premium plan) — same playbook, est. −$250–300/mo.**

**Docs updated in this change:** CLAUDE.md (architecture pattern + new CORS/lockdown/callback invariants + ACR row), ARCHITECTURE.md, HANDOFF.md, BETA_OPERATIONS_RUNBOOK.md (§2 429 playbook marked historical), VIDEO_INFRASTRUCTURE_RUNBOOK.md, GENE.md, `apim_policy.xml` + `bicep/apim/main.bicep` → tombstones (six-incident history preserved in git history).

**Rollback:** repoint FD route at `apim-origin-group` (APIM still running until post-soak deletion) + restore the cors policy from `apim_policy.xml` git history + remove the two platform-CORS origins. ACR: `az acr update --sku Standard`. Budgets: PUT the old $450 amounts back.

---

## PLAY OPEN BETA ENDED: "Join the beta" card on the Play listing — ⚠️ SUPERSEDED: the pause never took effect on the listing; see the 2026-07-06 top entry (was: ✅ RESOLVED, track paused 2026-07-03)

> **⚠️ CORRECTION (2026-07-06):** this entry was closed on the Console banner alone — the pause never persisted (managed publishing is OFF and nothing was queued, so it wasn't held; it just never saved). Three days later the listing still rendered a joinable "Join the beta" card (`stillbeta01.png`) and the API showed the beta track live with vc8 `completed`. API remediation could only HALT the latest release (vc8 halted, which resurfaced vc7 — full deactivation is impossible via the API) — see "PLAY 'JOIN THE BETA' STILL LIVE (third act)" above for the Console steps + opt-in-URL acceptance check. Lesson: verify on the live listing, not the Console banner.

**Symptom:** the public Google Play listing still showed a **"Join the beta"** opt-in card (screenshot `endbeta01.png`, since deleted) two weeks after vc9 went fully public on the production track.

**Root cause (verified via Play Developer API `get_releases`, 2026-07-03):** the **open testing (beta) track was never paused** at the 2026-06-18 production release — it still carries a live release (`beta: completed, 100%, versionCode 8`; alpha/internal are inactive drafts). An active open-testing track with a public opt-in is what renders the "Join the beta" card. **Not a broken-build exposure:** Play serves the highest eligible versionCode, so a joiner receives production vc9, not the stale vc8 (which predates the real RevenueCat `goog_` key).

**Fix:**
- ✅ **Gene (Play Console, manual) — DONE 2026-07-03:** Open testing → **"Pause track"** — Console confirmed "This track is paused / Testers are not receiving this release", track summary **Inactive**, vc8 release left in place (screenshot-verified). Card removal from the public listing now just propagates (allow up to ~24h + Play Store app cache). Effects: card disappears from the listing (propagation up to ~24h + Play Store app cache); existing testers keep the app and continue getting production updates (vc9 ≥ vc8); opt-in link stops accepting joins. Reversible via "Resume track" (tester list preserved); for future pre-release validation prefer **internal testing** (no public listing footprint).
- ✅ **Web (shipped 2026-07-03):** "Now in beta" chips removed from the landing page — `BetaChip` deleted from `LandingNav.tsx`, `Hero.tsx` (platforms line "iOS · Android · Web" kept, now always visible), `Download.tsx`; `BetaChip.tsx` component deleted. Lint clean + build green; auto-deploys on merge (SWA).

**Runbook lesson (add to any future go-public checklist):** **releasing to production does NOT end open testing.** The open-testing track keeps rendering its public "Join the beta" card until the track is explicitly paused.

**Verification:** re-check the listing from a non-enrolled account (the `endbeta01.png` account saw "Join", so it's a valid control) a few hours after pausing; confirm clique-pix.com shows no beta chips after the SWA deploy; one tester device should still run vc9 normally.

---

## 🎉 iOS APPROVED + PUBLIC: 1.0 (12) passed Apple review — CLIQUE Pix live on BOTH stores (2026-07-03)

**Status:** ✅ **Apple approved iOS 1.0 (12) on 2026-07-02** — the first public App Store release, closing out three rejection rounds (1.0(9) DOB/SIWA/tracking/IAP-not-submitted → 1.0(10) IAP screenshots + Entra footer → 1.0(11) IAP promotional images). Gene built the +12 IPA on the Mac, uploaded via Transporter, and resubmitted after deleting the promotional images; approval email arrived 2026-07-02. **Listing verified live** via `https://itunes.apple.com/lookup?id=6766294274` (`resultCount: 1`): store name **"CLIQUE Pix: Group Pic Sharing"**, URL `https://apps.apple.com/us/app/clique-pix-group-pic-sharing/id6766294274`. Combined with Android (public since 2026-06-18), **CLIQUE Pix is now publicly available on iOS, Android, and web.**

**Repo follow-through shipped same-day (2026-07-03):**
- **Smart App Banner (Phase C-final) ACTIVATED** per `docs/INVITE_INSTALL_REFERRER.md`: `<meta name="apple-itunes-app" content="app-id=6766294274">` in `webapp/index.html` + `app-argument` rewrite in `InviteAcceptScreen.tsx` — iOS invite codes now ride into the freshly installed app via `NSUserActivity.webpageURL` on the Smart App Banner "OPEN" tap.
- **Invite install banner flipped TestFlight → App Store**: `InstallBanner.tsx` iOS branch now renders `AppStoreBadge` (canonical listing URL) and the "retap your invite link" caption is dropped; `TestFlightBadge.tsx` deleted (unused). `AppStoreBadge` gained an `onClick` prop so the `web_invite_install_badge_clicked {platform: 'ios'}` telemetry is preserved.
- **Landing page live badges**: `Hero.tsx` + `Download.tsx` App Store and Play badges now carry real store URLs (were `#` placeholders); "links coming soon" / "iOS launching soon" copy replaced with "Available now".
- **Docs flipped**: `HANDOFF.md` launch status (iOS → public), `INVITE_INSTALL_REFERRER.md` (C-final shipped, C-interim superseded), `GENE.md`, this entry. Rejection working files (`applerejection01.txt`, `Applereject02.txt`, the two rejection PNGs, `IMG_0920.jpeg`) deleted (were untracked).
- Verification: webapp `npm run lint` clean + `npm run build` green. Web deploys automatically on merge to `main` (SWA).

**Remaining post-launch tail (Gene, manual — none new):** revert the RC service-account Admin grant → least-privilege; Android tester 1-year promos in RC; **trial clock: extended trials expire 2026-07-11** — testers without grants hit the paywall then (both stores can now take real subscriptions, so this is a soft deadline); store screenshots still showing the old "Clique Pix" wordmark (design task, GENE.md).

---

## THIRD APPLE REJECTION: iOS 1.0 (11) rejected on 2.3.2 (IAP promotional images) — ✅ RESOLVED: 1.0 (12) APPROVED 2026-07-02 (2026-07-02)

**Status:** ❌ iOS **1.0 (11)** REJECTED (same Submission ID `7d415cdc-c200-45d5-94bd-93ee7c11d979` — Apple threads resubmissions under one submission; reviewed June 30 2026, iPhone 17 Pro Max). **ONE finding, metadata-only — no code, no binary, no Entra work.** Both 1.0(10) findings are confirmed cleared: the letter opens "Thank you for your resubmission" and re-raises neither 2.1(b) (App Review Screenshots held — the subscriptions were actually reviewed this time) nor 2.1(a) (Entra footer fix held). Rejection letter: `Applereject02.txt` (repo root, uncommitted working file).

| Finding | Root cause | Fix | Owner |
|---|---|---|---|
| **2.3.2** promotional image "is the same as the app's icon" + "duplicate or identical promotional images for different promoted In-App Purchase products" | While uploading the required **App Review Screenshots** for the 2.1(b) fix (2026-06-29), the app icon was ALSO uploaded into each subscription's **App Store Promotion → Promotional Image** slot — an *optional, public-facing* field for promoting IAPs in App Store search/featuring. Same icon on both subs tripped both bullets. | **Delete the promotional image from both subscriptions** (Apple explicitly blesses deletion when not promoting IAPs: "If you have no future plans on promoting this In-App Purchase product, you can delete the associated promotional image"). ✅ **DONE 2026-07-02.** App Review Screenshots left untouched. | Gene (ASC, manual) |
...
**Resolution (2026-07-02):** ✅ build **1.0 (12)** (pubspec bumped `+11 → +12`, code-identical to 11 — the binary was never faulted; fresh number chosen over re-attaching build 11) built on the Mac, uploaded via Transporter, submitted with the promo images deleted → **APPROVED same day.** See the 2026-07-03 top entry.
...
**Positive signals from the review thread (worth recording):** the 1.0(10) rejection attachments (`docs/Screenshot-0624-170104.png` / `-170206.png`, uncommitted working files) show the reviewer signed up via **Sign in with Apple with a private-relay email** on an iPad Air 11" M3 and landed in the fully working app (Profile screen, Manage Subscription visible) — SIWA + fresh-account 7-day trial + iPad rendering all passed real review. The 2.1(a) screenshot confirms the "error" was the Entra footer's Troubleshooting-details panel showing benign code 50058 (silent-SSO, no session cookie — expected under the ephemeral ASWebAuthenticationSession design).

---

## SECOND APPLE REJECTION: iOS 1.0 (10) rejected on 2.1(b) + 2.1(a) — ❌ 1.0 (11) REJECTED ON 2.3.2 (see top entry) (2026-06-29)

**Resubmission (2026-06-29):** ✅ both fixes applied + **1.0 (11) built, uploaded via Transporter, and submitted for review.** 2.1(b): App Review Screenshot (real-priced paywall captured on-device, `$3.99`/`$39.99`) uploaded to BOTH subscriptions (`plus_monthly`/`plus_annual`) + reviewer notes updated; both attached to the version. 2.1(a): Entra external-tenant sign-in **footer hidden** (Company branding → Default sign-in → Layout → "Show footer" off) — verified the troubleshooting "…" ellipsis is gone from `cliquepix.ciamlogin.com`. Build 11 = identical app code to 10 (both findings fixed OUTSIDE the app); pubspec `1.0.0+11` (PR #72, merged). **On-submission check: the two subscriptions must show "Waiting for Review"/"In Review", NOT "Ready to Submit".** Capture method for the screenshot: forced a test account non-entitled via psql (`UPDATE users SET entitlement_active=FALSE, trial_ends_at=NOW()-INTERVAL '1 day' WHERE id=...`), screenshotted the real paywall, then restored the trial.

**Status:** ❌ iOS **1.0 (10)** REJECTED (Submission `7d415cdc-c200-45d5-94bd-93ee7c11d979`, reviewed June 25 2026, iPad Air 11-inch M3, iPadOS 26.5). Two findings. **The 1.0(9) findings (DOB 5.1.1(v), SIWA Guideline 4, tracking 5.1.2(i)) are confirmed cleared** — the reviewer signed up via Sign in with Apple straight into a working app (no re-prompt). Full step-by-step runbook: `C:\Users\genew\.claude\plans\how-do-i-get-cozy-hejlsberg.md`.

**⚠️ CORRECTION to prior entries:** the 2026-06-23/24 entries below + `GENE.md` marked **2.1(b) ✅ "IAPs attached + screenshot + new binary submitted together."** That was NOT actually true in App Store Connect — Apple rejected on the identical point. **Lesson: do not mark an ASC IAP "submitted" until its status literally reads "Waiting for Review."** "Ready to Submit" = NOT submitted.

| Finding | Root cause | Fix | Owner |
|---|---|---|---|
| **2.1(b)** IAPs not submitted for review (REPEAT) | The subscriptions never got an **App Review Screenshot** (mandatory to submit an IAP), so they stayed "Ready to Submit" and the binary submitted without them. | Upload a paywall screenshot to each subscription's App Review Information → attach both to the 1.0 version → confirm status flips to **"Waiting for Review"** → submit binary **1.0 (11)** WITH the IAPs. | Gene (ASC, manual) |
| **2.1(a)** "app displayed an error when tapped the three dots on the bottom left of the login screen" | The "…" is the **Microsoft Entra hosted sign-in page's troubleshooting-details ellipsis** (a footer element). Error **50058** = "silent sign-in request, no session cookie" — benign, and an expected consequence of the ephemeral ASWebAuthenticationSession (`Broker.msAuthenticator`). It blocks nothing (reviewer signed in fine 60s earlier). | **Entra admin center → external (CIAM) tenant → Company branding → Default sign-in → Layout → uncheck "Show footer"** (MS docs confirm the troubleshooting ellipsis is a footer element). No code, no binary. Do NOT "fix" via persistent cookies — that reintroduces the "Continue as previous user" account-switch trap. | Gene (Entra, manual) |

**Capturing the App Review Screenshot (the piece Gene didn't know how to produce):** the paywall is only reachable by an account with NO access (no subscription + lapsed/zero trial) — entitled/in-trial users are bounced off `/paywall`. Chosen method: sign up a throwaway account on the TestFlight build, force it non-entitled via psql (`UPDATE users SET entitlement_active=FALSE, trial_ends_at=NOW()-INTERVAL '1 day' WHERE id='<uuid>'`), screenshot the real paywall on the device, then restore the trial. Backstop if iOS products don't render: RevenueCat dashboard paywall preview (`pw9ac01d9e31184633`) — but it shows placeholder prices ($9.99/$69.99), not the real $3.99/$39.99. Apple's review screenshot is review-only and just must "clearly show the item/service offered."

**Also owed in the same resubmission:** App Review Notes explaining the reviewer's comped account never naturally hits the paywall (so the screenshot is how they see the offer); optionally a sandbox tester for purchase testing.

---

## APPLE REJECTION REMEDIATION: remove DOB age gate (iOS 1.0(9) → (10)) — ⏳ REPO DONE, PORTAL/STORE WORK OWED (2026-06-23)

**Status:** ✅ repo code + docs complete (backend tsc clean + **197 jest**; Flutter `analyze` 54-baseline + **120 tests**; webapp lint + build green; privacy policy updated, effective 2026-06-23). ⏳ Manual portal/store work owed by Gene (Entra, App Store Connect, Play) before resubmission — **that is what actually clears the rejection.**

**Why.** Apple rejected iOS build **1.0 (9)** (Submission `f39d9a0a-43af-40c4-9a01-fef7524f572a`, reviewed 2026-06-23) on four findings: **5.1.1(v)** app requires Date of Birth (not core-relevant); **Guideline 4** Sign in with Apple re-asks name/email; **5.1.2(i)** App Privacy declares tracking but no ATT prompt; **2.1(b)** subscription IAPs not submitted with the binary. The 13+ gate was self-imposed policy (not legal), so DOB collection was removed entirely; 13+ is now a stated eligibility line in Terms only ("stated policy only").

**What shipped (repo).**
- **Backend:** removed all age-gate code from `auth.ts` (`decideAgeGate`/`extractDobFromClaims`/`parseAnyDob`, the 403 `AGE_VERIFICATION_FAILED` block, the `age_verified_at` upsert, `age_gate_*` telemetry); deleted `ageUtils.ts` + both age-gate test files; dropped `age_verified_at` from the `User` TS model. **`users.age_verified_at` column left in place (dead); migration 008 untouched.** `entraGraphClient.ts` kept (uncalled, earmarked v1.5). A missing DOB claim was always a "grandfather → pass," so sign-in cannot break in any deploy order.
- **Clients:** removed the dead `AGE_VERIFICATION_FAILED` 403 handling (Flutter `auth_providers.dart`; web `client.ts` + the now-unused `errorCode`/`errorMessage`), the dead web `ageVerified?` model field, and the stale `useAuthVerify.ts` comment. Web LoginScreen 13+ disclaimer kept (still accurate). pubspec `1.0.0+9 → +10`.
- **Legal:** `webapp/public/docs/privacy.html` §2.2 (dropped DOB-collection paragraph) + §11 (removed the verify/auto-delete enforcement claims; kept notice-based children's-privacy), effective date → 2026-06-23. `terms.html` §2 already notice-based — unchanged.
- **Docs:** CLAUDE.md (Age Gate section replaced with a don't-re-introduce-DOB guardrail; fixed the stale `website/` path), ARCHITECTURE.md, AUTHENTICATION.md, PRD.md, WEB_CLIENT_ARCHITECTURE.md, BETA_TEST_PLAN.md, HANDOFF.md updated; AGE_VERIFICATION_RUNBOOK.md marked **deprecated**.

**Verified no tracking (5.1.2(i)):** no `NSUserTrackingUsageDescription`/`SKAdNetworkItems`, `firebase_analytics` absent, `IS_ANALYTICS_ENABLED=false` / `firebase_analytics_collection_enabled=false`, RevenueCat billing-only, web is first-party App Insights → fix is the ASC App Privacy label only ("Does not track"); **no ATT code needed.**

**Manual / store work owed (Gene — clears the rejection):**
1. **Entra portal:** remove the `dateOfBirth` custom attribute from the `SignUpSignIn` user flow + stop the claim. **Also verify the flow does NOT collect/require name or email after Sign in with Apple** — Guideline 4 is fixed ONLY here; DOB removal alone does not fix it. Watch Apple private-relay email.
2. **ASC App Privacy:** set the offending data type to NOT "used to track you"; declare "Does not track."
3. **ASC IAP (2.1(b)):** attach `plus_monthly` + `plus_annual` to the version + IAP App Review screenshot + upload the new binary (1.0 (10)) + submit.
4. **ASC Age Rating + Play content rating:** confirm neither asserts in-app age verification.
5. **Play Console Data Safety:** remove DOB collection + parental-consent declaration (Android is public on the same Entra flow + shared privacy policy).

**Sequencing:** Entra change first (sign-in survives via grandfather) → merge the repo PR so the web SWA redeploys updated privacy/terms + flip Play Data Safety in the same window (avoid a live policy-vs-collection mismatch) → ASC App Privacy → submit IAPs + new binary.

**Rollback:** repo changes are a single PR (`git revert`); the dead column + migration 008 are untouched, so no DB rollback. Re-adding the gate would also require restoring the Entra attribute.

---

## ANDROID PUBLIC RELEASE: vc9 → production track — ✅ APPROVED + PUBLIC (2026-06-18)

**Status:** ✅ **vc9 passed Google review and is fully released to the public on the production track at 100% rollout (2026-06-18).** CLIQUE Pix's **first public release on Google Play** — and the first with working Android billing. Confirmed via Play Console → Publishing overview (was "In review", now published); Play Developer API (`get_releases` MCP) reports `production / versionCode 9 / completed / 100%`. Supersedes vc7 (uploaded 2026-06-10).

vc9 AAB was built clean (upload-key signed, `1.0.0+9`, 58.3 MB), pushed to the production track 2026-06-17, entered Google review, and was approved + released to the public 2026-06-18. Carries the real `goog_` RevenueCat key (PR #62) + the paywall never-blank hardening / stable-router fix.

**Note on rollout + smoke test:** the release published at **100% immediately** on approval (configured for 100%, not staged), and the team chose to go **straight to public** — the pre-100% License-testing purchase smoke test was **not performed / descoped**. Android billing is therefore validated by **real public traffic + RevenueCat webhooks** rather than a controlled test buy. If a billing issue surfaces in production, remediate via a **new release** (an already-100% rollout can't be reduced) and watch RevenueCat + `entitlement_*` webhook telemetry for purchase failures.
...
**Still owed:** revert the temporary Admin grant on the RC service account → least-privilege; Android tester promos (grantable in RC once a vc9 install creates their customers). *(The License-testing purchase smoke test is descoped — see "Note on rollout + smoke test" above; the app is public.)*

## iOS: App Store review — 🍎 IN REVIEW (2026-06-18)

**Status:** the iOS build (`1.0.0 (9)`, Apple ID `6766294274`, bundle `com.cliquepix.app`) is **submitted and in active Apple App Store review** as of 2026-06-18 — the move off TestFlight-only toward a public App Store listing. Built on the Mac (`flutter build ipa --release`, clean → `pub get` → `pod install` → build; automatic signing, team `4ML27KY869`) → `build/ios/ipa/clique_pix.ipa` (39 MB) → uploaded via **Transporter**. iOS billing was already wired (RevenueCat iOS key `appl_OvhNypnojnQSEebpQtBikJYTHBa` live; `plus_monthly` + `plus_annual` Ready to Submit, attached on the version page).

**New in this build:** the **branded iOS launch screen** (PR #67) — primary brand gradient (`#00C2D1 → #2563EB → #7C3AED`) + camera icon + white "CLIQUE Pix" wordmark, replacing Flutter's default placeholder. Cleared the "default placeholder launch image" build warning. Native storyboard + asset catalog only (no `flutter_native_splash`). Build + regen recipe: `HANDOFF.md §3 → "iOS launch screen (branded splash)"`.
...
- **Reviewer access:** account `vwhitley1967@gmail.com` (email+password, NOT SSO) has a verified **lifetime RevenueCat promotional grant** — the reviewer sees the full app past the paywall. Confirm the review notes carry the reviewer + sandbox-tester instructions.
- **Paywall guidelines:** §3.1.2 (subscription disclosures) + the live Privacy/Terms at `clique-pix.com/docs/*` are the URL-check requirements; both are live.
- When the public App Store listing goes live, activate the iOS deferred-deep-link Phase C-final (Smart App Banner) per `docs/INVITE_INSTALL_REFERRER.md`.

**On approval:** update this entry + `HANDOFF.md` launch status (iOS → public) and the invite-flow docs (TestFlight badge → App Store badge).
...
1. ~~**Smoke-test a purchase** on a versionCode-9 build with a Play license-test account~~ — **DESCOPED 2026-06-18:** vc9 went straight to public on Google's approval; no License-testing smoke test was run. Billing is now validated by real public purchases + RevenueCat webhooks.
...
**Status:** ✅ mitigated live (`az containerapp job update --min-executions 0`, verified — execution churn stopped at 2026-06-12T01:31Z) · ✅ docs updated (CLAUDE.md, VIDEO_ARCHITECTURE_DECISIONS.md Decision 12 revision, VIDEO_INFRASTRUCTURE_RUNBOOK.md) · ⏳ **Azure support ticket for the June 3-9 metering anomaly — full evidence package + filing steps now in `docs/MICROSOFT_CREDIT.md` (independently re-verified; ask $411.25, conservative floor $346.84, both excluding the legitimate ~$3.74/day idle baseline). File when the June MCA invoice posts (~July 5-9, 2026) — current Cost Management figures are estimated/unbilled, and you can't dispute an un-issued invoice. NOTE: MICROSOFT_CREDIT.md's re-verified metrics supersede a few figures below — executions were ~17,280/day (5s poll, not "every ~30s/2,600/day") and the single transcode was 19s FFmpeg (not 41.6s).**
