...
**Deploy order:** migration 016 via psql → backend `func publish` (accepts 336 before anything sends it) → merge to main (SWA auto-deploys web + legal + landing) → mobile rides the next build (+13), old builds covered by the shim indefinitely.
