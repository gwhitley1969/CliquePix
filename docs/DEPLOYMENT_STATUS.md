...
**Status:** ✅ mitigated live (`az containerapp job update --min-executions 0`, verified — execution churn stopped at 2026-06-12T01:31Z) · ✅ docs updated (CLAUDE.md, VIDEO_ARCHITECTURE_DECISIONS.md Decision 12 revision, VIDEO_INFRASTRUCTURE_RUNBOOK.md) · ⏳ **Azure support ticket for the June 3-9 metering anomaly — full evidence package + filing steps now in `docs/CREDIT.md` (independently re-verified; ask $411.25, conservative floor $346.84, both excluding the legitimate ~$3.74/day idle baseline). File when the June MCA invoice posts (~July 5-9, 2026) — current Cost Management figures are estimated/unbilled, and you can't dispute an un-issued invoice. NOTE: CREDIT.md's re-verified metrics supersede a few figures below — executions were ~17,280/day (5s poll, not "every ~30s/2,600/day") and the single transcode was 19s FFmpeg (not 41.6s).**
