...
**Deploy status (2026-07-05):** ✅ backend `func publish` DONE (health 200 — deploying before the migration is safe: nothing sends 336 until the web merge; current clients send 24/72/168, all valid) · ⛔ **migration 016 BLOCKED from the dev box** — the current network (egress 98.22.9.190) filters TLS on port 5432 (TCP connects, TLS handshake killed; `sslnegotiation=direct` also chopped; PgBouncer not supported on Burstable). Unblock: run from another network (phone hotspot + retry) or Azure Cloud Shell (the `AllowAllAzureServices` firewall rule already permits it). → **MERGE GATE: do not merge PR #74 until 016 is applied** (merging auto-deploys web, which starts sending 336 → 500s against the old CHECK). Mobile rides the next build (+13); old builds covered by the shim indefinitely.
