...
- Video transcoding pipeline: TWO paths chosen by `canStreamCopy(probeResult)`:
  ...
  - Runs in Container Apps Job with `jrottenberg/ffmpeg:6-alpine` base image. KEDA polling 5s, **min-executions=0** (scale-to-zero; reverted from 1 on 2026-06-11 — min-executions=1 spawned an empty-poll execution every ~30s 24/7, ~$110-150/month idle burn, and was the substrate for a June 3-9 Azure metering anomaly. See `docs/VIDEO_ARCHITECTURE_DECISIONS.md` Decision 12 revision note. Do NOT set back to 1 to chase cold-start latency — the v1.5 answer is the Container Apps Service migration.)
...
- Reviewer + beta testers: RevenueCat **Promotional** entitlement grants (no DB override of `entitlement_active`; extending `users.trial_ends_at` via SQL is the sanctioned emergency unblock — it IS the trial mechanism).
...
- **Hard rule (2026-06-11 lockout incident): `PaywallView` must ONLY be mounted behind the `paywallOfferingProvider` pre-flight** (`paywall_providers.dart`: configure → `isConfigured` → `getOfferings()` 10s timeout → current offering with ≥1 package). It is a bare platform view with NO load-failure callback — mounted with an unconfigured SDK (e.g. a placeholder key) or unloadable offerings it renders a BLANK SCREEN, which hard-locked every lapsed-trial Android user out of the app. Pre-flight failures render the branded `_PaywallFallback` (Try Again / Refresh subscription status / Manage account). `RevenueCatService.configure()` short-circuits placeholder keys (`RevenueCatConstants.isPlaceholderKey`) and exposes `isConfigured`/`configureError` + `revenuecat_configure_failed` telemetry — keep failures observable.
- **Hard rule: the RC webhook resolves the user as the FIRST valid-UUID among `app_user_id` → `original_app_user_id` → `aliases[]`** (`entitlementService.upsertEntitlement`). RC pins `original_app_user_id` to the customer's FIRST id — `$RCAnonymousID:…` forever for SDK-created customers — so preferring it dropped EVERY webhook for anonymous-origin customers (incl. promotional grants, the reviewer mechanism). Do not "simplify" back to a single field.
- **Hard rule: `revenuecatRestClient.ts` must stay on RC API v2.** The Key Vault secret (`revenuecat-secret-api-key`) is a v2 project key; RC v1 endpoints reject it with error 7723. The original v1 client failed invisibly on every production call (forceSync + GDPR delete are best-effort and swallow errors).
- **Hard rule: `routerProvider` must NOT `ref.watch` auth/access state.** Recreating the GoRouter resets navigation to `initialLocation` (verified in `app/test/router_recreation_behavior_test.dart`). The router is created once per signed-in identity (watches only `currentUserIdProvider` for the cross-user tab-stack reset); redirect re-evaluation is driven by `refreshListenable` with `ref.read` inside the redirect. The paywall-exempt route set includes `/invite/*` + `/diagnostics` prefixes. `DeepLinkService` takes a router *getter*, never a captured instance.
...
- **Operational gotcha:** the SWA Free tier caps staging environments at 3 concurrent. Every PR creates one. Cleanup is **automated** by `.github/workflows/swa-cleanup.yml` (triggers on `pull_request: types: [closed]`, runs the SWA `action: close`, SHA-pinned) — see `docs/WEB_CLIENT_ARCHITECTURE.md §13.1`. The old in-workflow close job never fired because `webapp-deploy.yml`'s `pull_request` trigger omitted `types:` (which defaults to `[opened, synchronize, reopened]`, excluding `closed`) — fixed PR #29, 2026-06-05. Manual `az staticwebapp environment delete --name swa-cliquepix-prod --resource-group rg-cliquepix-prod --environment-name <PR#> --yes` is the break-glass fallback if the cap is ever hit.
...
Client does **validation only** (no meaningful client compression): extension MP4/MOV, duration ≤5min, file-size estimate, show upload-time estimate. Server: ffprobe authoritative validation → `canStreamCopy(probeResult)` picks **fast path** (`-c copy` tee remux, ~2–5s) vs **slow path** (libx264 re-encode + HDR→SDR `zscale`/`tonemap=hable`). Both emit HLS + MP4 from a **single tee-muxer invocation**; poster extracted separately; original master set to Cool tier. KEDA polling 5s, min-executions=0 (scale-to-zero since 2026-06-11 — see Decision 12 revision note).
